...
def main_worker(gpu_idx, configs):
    ...
    configs.device = torch.device('cpu' if configs.gpu_idx is None else 'cuda:{}'.format(configs.gpu_idx)) # quindi 'cuda:0'
    ...
    for epoch in range(configs.start_epoch, configs.num_epochs + 1):
        ...
        if not configs.no_val: # se vogliamo utilizzare il validation set
            val_dataloader = create_val_dataloader(configs)
