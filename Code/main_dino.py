...
def train_dino(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationDINO(args.global_crops_scale, args.local_crops_scale, args.local_crops_number,) # it returns the list of the 10 cropped images of the same one (2 'global' and 8 'local')
    ...
    # ============ preparing optimizer ... ============
    params_groups = utils.get_params_groups(student) # just a way to separate regularized and not regularized model parameters
    ...
    for epoch in range(start_epoch, args.epochs): # if we have a trained model until the start_epoch (=20) and we want to train it for 10 more, args.epochs has to be 30!
        ...
        # Every epoch we update the 'checkpoint.pth' file!
        ...
        # Every epoch we write the train stats on a log file! --> these are the metrics that we're interested in for logging with WandB!
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
                    ...
                    fp16_scaler, args):
    metric_logger = utils.MetricLogger(delimiter="  ") # very interesting artifact
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)): # 'log_every' also returns batches of training images
...
class DINOLoss(nn.Module): # loss used as a model :)
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
        ...
        self.register_buffer("center", torch.zeros(1, out_dim)) # a buffer is not trained by the optimizer!
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        student_out = student_out.chunk(self.ncrops) # it becomes a list of  tensor, where each tensor is a softmax distribution over the out_dim
        ...
        total_loss = 0
        n_loss_terms = 0
        for iq, q in enumerate(teacher_out): # iq in {0,1}
            for v in range(len(student_out)): # v in [0...9]
                if v == iq:
                    # we skip cases where student and teacher operate on the same view (the same crop image) --> we aoid the two networks to work both on the same global views
                    continue
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number): # ( (0.4, 1.), (0.05, 0.4), 8 )
