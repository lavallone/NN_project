...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, model_name, patch_size): # in case I want to show the paper's results
...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    checkpoint = torch.load(ckp_path, map_location="cpu") # It returns a state_dict that is a Python dictionary
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            yield obj # it returns a batch of images
...
class MultiCropWrapper(nn.Module): # this is the actual neural network architecture used by the two architectures (BACKBONE + HEAD)
    """
    ...
    def forward(self, x):
        # convert to list
        if not isinstance(x, list): # if it's not a list
            x = [x]
        idx_crops = torch.cumsum(
                    torch.unique_consecutive(torch.tensor([inp.shape[-1] for inp in x]), return_counts=True,)[1],
                    0) # it'll be  equal to [2] or to [2, 10] (it depends if is the teacher or the student network)
        start_idx, output = 0, torch.empty(0).to(x[0].device) # we simply initialized the output tensor saving it to the gpu
        for end_idx in idx_crops: # it's neededd to separate  the computations of local and global cropped images
